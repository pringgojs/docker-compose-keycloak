...
<html lang="id">
  <head>
    ...
    <title>Update Password - SSO</title>
  ...
  <body class="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center px-4">
    ...
    <div class="max-w-md w-full bg-white dark:bg-gray-800 shadow-2xl rounded-2xl p-8 relative z-10">
      ...
      <div class="text-center mb-6">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Silakan masukkan password baru Anda</p>
      </div>
      <#if message?has_content>
        <div class="mb-4 text-red-600 dark:text-red-400 font-medium text-sm">${message.summary}</div>
      </#if>
      <form id="kc-update-password-form" method="post" action="${url.loginAction}" class="space-y-4">
        <input type="hidden" name="credentialId" value="${credentialId!}"/>
        <div>
          <label for="password-new" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Password Baru</label>
        ...
        <div>
          <label for="password-confirm" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Konfirmasi Password</label>
